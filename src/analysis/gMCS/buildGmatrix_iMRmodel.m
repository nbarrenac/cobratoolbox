function [G, G_ind, related, n_genes_KO, G_time, iMRmodel_info] = buildGmatrix_iMRmodel(model_name, model_struct, regulatoryNetwork, nLayers)
...
if ~exist([tmpFolderName filesep 'eGPR_models'],'dir')
    mkdir([tmpFolderName filesep 'eGPR_models'])
end
   ...
   n_mcs = 1e11;%100000000000;
    ...
    for i = 1:length(uniqueGPR)
        ...
        if uniqueGPR(i)~=""
            ...
            if  ~isempty(find(ismember(table2cell(regulatoryNetwork(:,2)), metGenes{i,1}),1)) %if any ot the genes in the GPR rule is in the signaling network as targets
                for layer = 1:nLayers
                    ...
                    if layer == 1
                        ...
                        new_network = table2cell(regulatoryNetwork(ismember(table2cell(regulatoryNetwork(:,2)), genes_sel),:));
                    else
                    ...
                    if size(network_pre,1) == size(network_2,1)
                        ...
                        if size(network_2,1) == 1
                            iMRmodel_info.genes_eGPR{i} = unique([unique([network_2(:,1) network_2(:,2)]'); metGenes{i,1}']);
                        else
                            iMRmodel_info.genes_eGPR{i} =  unique([unique([network_2(:,1) network_2(:,2)]); metGenes{i,1}']);
                        end
                    ...
                    if sol.stat ~= 0
                        ...
                        if size(network_pre,1)==1
                            iMRmodel_info.genes_eGPR{i} = unique([unique([network_pre(:,1) network_pre(:,2)]'); metGenes{i,1}']);
                        elseif size(network_pre,1)==0
                            iMRmodel_info.genes_eGPR{i} = {};
                        else
                            iMRmodel_info.genes_eGPR{i} =  unique([unique([network_pre(:,1) network_pre(:,2)]); metGenes{i,1}']);
                        end
                        iMRmodel_info.reason(i) = 2;
                        network_2 = network_pre;
                        break
                    else
                        iMRmodel_info.layersPer_eGPR(i) = layer;
            ...
            if size(network_2,1)==1
                iMRmodel_info.genes_eGPR{i} = unique([unique([network_2(:,1) network_2(:,2)]'); metGenes{i,1}']);
            elseif size(network_2,1)==0 && ~isempty(metGenes{i,1})
                iMRmodel_info.genes_eGPR{i} = metGenes{i,1}';
            else
                iMRmodel_info.genes_eGPR{i} =  unique([unique([network_2(:,1) network_2(:,2)]); metGenes{i,1}']);
            end
            for j = 1:size(network_2,1)
                if cell2mat(network_2(j,3)) == 1
                    BooleanRules(j+1,1) = strcat(network_2(j,2), ' = ', {' '}, network_2(j,1));
                elseif cell2mat(network_2(j,3)) == -1
                    BooleanRules(j+1,1) = strcat(network_2(j,2), ' = !', network_2(j,1));
                end
            end
